...
// Committer is the committer of an inscription
...
func (b *Committer) BlockChainInfo() (*btcjson.GetBlockChainInfoResult, error) {
...
// NewInscriptionTool creates a new InscriptionTool
...
func (tool *InscriptionTool) buildCommitTx(commitTxOutPointList []*wire.OutPoint, totalRevealPrevOutput, commitFeeRate int64) error {
	...
	for i := range commitTxOutPointList {
		log.Println("point:" + commitTxOutPointList[i].String())
...
func (tool *InscriptionTool) getTxOutByOutPoint(outPoint *wire.OutPoint) (*wire.TxOut, error) {
	...
	tx, err := tool.client.GetRawTransactionVerbose(&outPoint.Hash)
	if err != nil {
		return nil, err
	}
...
func (tool *InscriptionTool) BackupRecoveryKeyToRPCNode() error {
	...
	descriptors := make([]Descriptor, len(tool.txCtxDataList))
	for i := range tool.txCtxDataList {
	...
	results, err := ImportDescriptors(tool.client, descriptors)
	if err != nil {
		return err
	}
	if results == nil {
